import torch
import peps.groups.su2 as su2
import peps.config as cfg
from peps.ctm.generic.env import ENV
from peps.ctm.generic import rdm
from peps.ctm.generic import corrf
from peps.ctm.one_site_c4v.env_c4v import ENV_C4V
from peps.ctm.one_site_c4v import rdm_c4v
from peps.ctm.one_site_c4v.rdm_c4v_specialized import rdm2x2_NNN_tiled,\
    rdm2x2_NN_tiled, rdm2x1_tiled
from peps.ctm.one_site_c4v import corrf_c4v
...
class J1J2():
    def __init__(self, j1=1.0, j2=0, global_args=cfg.global_args):
        ...
        self.h2, self.SS_rot, self.h2x2_nn, self.h2x2_nnn, self.h2x2_nn_1site_stripe, self.h2x2_nnn_1site_stripe, \
        self.h2x2_nn_rot, self.h2x2_nnn_rot = self.get_h()
    ...
    def get_h(self):
        ...
        # 2x2 sublattice ansatz and bipartite ansatz
        ...
        # sublattice rotation for single local tensor stripe ansatz
        h2x2_nn_1site_stripe = torch.einsum('irktaxcy,jr,lt,xb,yd->ijklabcd',\
            h2x2_nn,rot_op,rot_op,rot_op,rot_op)
        h2x2_nnn_1site_stripe = torch.einsum('irktaxcy,jr,lt,xb,yd->ijklabcd',\
            h2x2_nnn,rot_op,rot_op,rot_op,rot_op)
        h2x2_nn_1site_stripe= h2x2_nn_1site_stripe.contiguous()
        h2x2_nnn_1site_stripe= h2x2_nnn_1site_stripe.contiguous()
        ...
        return SS, SS_rot, h2x2_nn, h2x2_nnn, h2x2_nn_1site_stripe, h2x2_nnn_1site_stripe, h2x2_nn_rot, h2x2_nnn_rot
    ...
    def energy_2x2_1site_stripe(self,state,env):
        r"""
        :param state: wavefunction
        :param env: CTM environment
        :type state: IPEPS
        ...
        energy_nnn=0
        tmp_rdm= rdm.rdm2x2((0,0),state,env)
        energy_nn += torch.einsum('ijklabcd,ijklabcd',tmp_rdm,self.h2x2_nn_1site_stripe)
        energy_nnn += torch.einsum('ijklabcd,ijklabcd',tmp_rdm,self.h2x2_nnn_1site_stripe)
        energy_per_site= 2.0*(self.j1*energy_nn/4.0 + self.j2*energy_nnn/2.0)
        energy_per_site= _cast_to_real(energy_per_site)
    ...
    def eval_obs_1site_stripe(self,state,env):
        ## ToDo
        obs= dict({"avg_m": 0.})
        with torch.no_grad():
            for coord,site in state.sites.items():
