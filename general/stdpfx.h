...
#define WRN(msg) { std::cout << "WRN: " << append_space(msg) << VENUE << std::endl; if (debug) suspend(); }
...
inline Str present_time_compact_format()
{
	time_t timer = time(0);
	tm *t = localtime(&timer);
	char buf[64];
...
#ifdef _MSC_VER
inline Int direxist(const Str &dir) { return _access(dir.c_str(), 0) != -1; }
#else
inline Int direxist(const Str &dir) { return access(dir.c_str(), 0) != -1; }
#endif
...
template<typename T> inline T ARG(const T &a) { return 0; }
