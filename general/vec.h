...
public:
	...
	inline Vec &reset(Idx n_i, T *p);					// reset to status after invoke Vec(Idx n_i, T *p)
	inline Vec &reset(const Mat<T> &m);					// reset to status after invoke Vec(const Mat<T> &m)
...
template<typename T>
Vec<T> &Vec<T>::reset(const Mat<T> &m)
{
	this->reset(m.size(), m.p());
	return *this;
}
...
inline Vec<T> concat(const Vec<T> &lhs, const Vec<T> &rhs)
{
	...
	Vec<T>(lhs.size(), ret.p()) = lhs;
	Vec<T>(rhs.size(), ret.p() + lhs.size()) = rhs;
	return ret;
}
...
#endif
	VecCmplx r(a.size());
	for_Idx (i, 0, r.size()) r[i] = cmplx(a[i], b[i]);
	return r;
}
...
{
	VecCmplx r(a.size());
	for_Idx (i, 0, r.size()) r[i] = cmplx(a[i], 0.);
	return r;
}
